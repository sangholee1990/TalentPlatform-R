...
# config
globalVar$refResConfig = "E://04. TalentPlatform//Github//TalentPlatform-R//REF-RESOURCES"

globalVar$config = getwd()
# globalVar$config = "D:/04. TalentPlatform/Github/TalentPlatform-R"
globalVar$resConfig = paste(globalVar$config, "RESOURCES", sep = '/')
globalVar$inpConfig = paste(globalVar$resConfig, 'INPUT', prjName, sep = '/')
globalVar$figConfig = paste(globalVar$resConfig, 'FIG', prjName, sep = '/')
globalVar$outConfig = paste(globalVar$resConfig, 'OUTPUT', prjName, sep = '/')
globalVar$logConfig = paste(globalVar$resConfig, 'LOG', prjName, sep = '/')
globalVar$mapConfig = paste(globalVar$resConfig, 'CONFIG', 'MAP-INFO', sep = '/')
globalVar$systemConfig = paste(globalVar$resConfig, 'CONFIG', 'system.cfg', sep = '/')
globalVar$seleniumConfig = paste(globalVar$resConfig, 'CONFIG', 'SELENIUM', sep = '/')
globalVar$fontConfig = paste(globalVar$resConfig, 'CONFIG', 'FONT-INFO', sep = '/')

# 기본 설정 복사
isDir = dir.exists(path = globalVar$resConfig)
isRefDir = dir.exists(path = globalVar$refResConfig)

if (isDir == FALSE & isRefDir == TRUE) {
  sprintf("[복사] 기본 설정 : cp %s %s", globalVar$refResConfig, globalVar$resConfig)

  dir.create(globalVar$resConfig)
  file.copy(globalVar$refResConfig, globalVar$config, recursive = TRUE)
}
