...
# 평균식분도에 대한 최대값 : "20230204_14개신라픽" 폴더 참조
# 모집단78개 : 0.781607143522527
# 경주지점 : 0.697976397970507
...
for (bootNum in bootNumList) {
  ...
  # 경주지점 0.69 이상
  bootDataL2 = bootData %>%
    # dplyr::group_by(xAxis, yAxis, meanVal, posVal) %>%
    dplyr::distinct(xAxis, yAxis, meanVal, posVal, keep_all = TRUE) %>%
    dplyr::filter(
      # posVal >= 0.68
      posVal >= 0.69
...
# 1. 위도 경도 그래프 : 20230203_경주식분0.68이상 폴더 참조 (점 개수 : 10000)
# 2. 위도 28~34/경도 110~116 구역안의 점 개수 : 104
# 3. 위도 34~42/경도 124~130 구역안의 점 개수 : 2634

# 1. 위도 경도 그래프 : 20230203_경주식분0.69이상 폴더 참조 (점 개수 : 10000)
# 2. 위도 28~34/경도 110~116 구역안의 점 개수 : 104
# 3. 위도 34~42/경도 124~130 구역안의 점 개수 : 2634

# 1. 위도 경도 그래프 : 20230203_붉은점0.78이상 폴더 참조 (점 개수 : 834)
# 2. 위도 28~34/경도 110~116 구역안의 점 개수 : 30
# 3. 위도 34~42/경도 124~130 구역안의 점 개수 : 116
