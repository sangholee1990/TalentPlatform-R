...
for (colorAlpha in colorAlphaList) {
  for (fontSize in fontSizeList) {
      ...
      makePlot = ggplot(dataL2, aes(x = Year, color = key, fill = key)) +
        # geom_bar(aes(x = interaction(Country, Year), y = Jobs_ths, color = NULL, group = interaction(Country)), stat = "identity", position = "dodge", alpha = colorAlpha) +
        geom_bar(aes(x = Year, y = Jobs_ths, color = NULL, group = interaction(Country)), stat = "identity", position = "stack", alpha = colorAlpha) +
        ...
        geom_line(data = dataL2, aes(x = interaction(Country, Year), y = val), linetype = "solid", size = 1, show.legend = FALSE) +
