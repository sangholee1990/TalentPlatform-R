...
- 분산 검정 결과 P값은 6.74e-08로서 유의수준 0.05 이하보다 작기 때문에 귀무가설 기각되어 항생제에 따른 차이가 있다.
...
- 항생제 종류 (Antibiotic)에 따라 데이터셋을 분할하여 4.2.2.1에서와 같이 결합 비율과 항생제 간의 피어슨 상관관계 검정을 수행함
- 다중 비교 과정에서 본페로니 교정을 통해 유의수준 조정도 고려해야 함
...
- 그러나 현재 Penic/Tetra 항생제의 경우 1~4번 소에 투여된 반면에 Strep/Eryth 항생제에서는 5~8번 소에 투여되었다.
