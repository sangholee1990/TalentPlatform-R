...
ggcorr(Final_dataset_Filtered, method = c("pairwise", "pearson"),
...
reg1 <- lm(GHG_Growth ~ Org.Crop.Share + Org.Liv.Share + Productivity + GDP_capita + Area.Org.Share, data = Final_dataset_Filtered)
reg2 <- lm(GHG_Growth ~ Liv.Prod.Org + Crop.Prod.Org + Productivity + GDP_capita + Area.Org.Share, data = Final_dataset_Filtered)
reg3 <- lm(Emissions ~ Org.Crop.Share + Org.Liv.Share + Productivity + GDP_capita + Area.Org.Share, data = Final_dataset_Filtered)
reg4 <- lm(GHG_Growth ~ Crop.Prod.Total + Liv.Prod.Total + Liv.Prod.Org + Crop.Prod.Org + Productivity + GDP_capita + Area.Org.Share, data = Final_dataset_Filtered)
reg5 <- lm(Emissions ~ Crop.Prod.Total + Liv.Prod.Total + Liv.Prod.Org + Crop.Prod.Org + Productivity + GDP_capita + Area.Org.Share, data = Final_dataset_Filtered)
...
ols_plot_resid_fit(reg2) # There seems to be localized heteroscedasticity
ols_test_breusch_pagan(reg3) #There is heteroscedasticity
ols_plot_resid_fit(reg3)
ols_test_breusch_pagan(reg4) #There is no heteroscedasticity
ols_test_breusch_pagan(reg5) #There is no heteroscedasticity



reg6 <- lm(Emissions ~ Crop.Prod.Total + Liv.Prod.Total + Productivity + GDP_capita + Area.Org.Share, data = Final_dataset_Filtered)
reg7 <- lm(Emissions ~ Liv.Prod.Org + Crop.Prod.Org + Productivity + GDP_capita + Area.Org.Share, data = Final_dataset_Filtered)

ols_test_breusch_pagan(reg6) 
ols_test_breusch_pagan(reg7) 

library(stargazer)
model_list <- list(reg1, reg2, reg3, reg4, reg5)
...
reg_final <- lm(Emissions ~ Crop.Prod.Total + Liv.Prod.Total + Liv.Prod.Org + Crop.Prod.Org + Productivity + GDP_capita + Area.Org.Share, data = Final_dataset_Filtered)
ols_test_breusch_pagan(reg_final)
summary(reg_final)
