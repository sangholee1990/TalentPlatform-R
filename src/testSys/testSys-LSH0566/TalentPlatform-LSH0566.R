...
# 엑셀 데이터 읽기
# 데이터 > 데이터 가져오기 > 데이터베이스에서
# 서버 localhost, 데이터베이스 sa
...
errorHandler = function(x) {
  ifelse(is.na(x) || length(x) == 0 || x == "", "NULL", x)
}
  ...
  for (uuidInfo in uuidList) {
    ...
    tryCatch(
      expr = {
        ...
        # 동적 쿼리
        selQuery = glue::glue_sql("
            SELECT uuidInfo
            FROM dbo.TB_EUDAMED
            WHERE uuidInfo = {uuidInfo}
        ", .con = con)
        
        # rs = DBI::dbSendQuery(con, selQuery)
        # result = DBI::dbFetch(rs)
        # if (DBI::dbHasCompleted(rs)) DBI::dbClearResult(rs)

        result = DBI::dbGetQuery(con, selQuery)
        ...
        data = data.frame(
          ...
          dplyr::mutate(
            regDate = format(Sys.time(), "%Y-%m-%d %H:%M:%S")
          )
        
        if (nrow(data) < 1) next
        
        # 동적 쿼리
        merQuery = glue::glue_sql("
      MERGE INTO dbo.TB_EUDAMED AS TAR
      USING (
        SELECT
      ...
      ON TAR.uuidInfo = SRC.uuidInfo
      WHEN MATCHED THEN
        UPDATE SET
          TAR.urlDtl = SRC.urlDtl,
          TAR.acrOrgName = SRC.acrOrgName,
          TAR.actIdSrn = SRC.actIdSrn,
          TAR.address = SRC.address,
          TAR.manDate = SRC.manDate,
          TAR.appLeg = SRC.appLeg,
          TAR.udiDu = SRC.udiDu,
          TAR.riskCls = SRC.riskCls,
          TAR.devName = SRC.devName,
          TAR.udiDi = SRC.udiDi,
          TAR.status = SRC.status,
          TAR.tradeName = SRC.tradeName,
          TAR.memberState = SRC.memberState,
          TAR.version = SRC.version,
          TAR.lastUpdData = SRC.lastUpdData,
          TAR.addProDesc = SRC.addProDesc,
      ...
      WHEN NOT MATCHED BY TARGET THEN
        ...
        VALUES (SRC.uuidInfo, SRC.urlDtl, SRC.acrOrgName, SRC.actIdSrn, SRC.address, SRC.manDate, SRC.appLeg, SRC.udiDu, SRC.riskCls, SRC.devName, SRC.udiDi, SRC.status, SRC.tradeName, SRC.memberState, SRC.version, SRC.lastUpdData, SRC.addProDesc, SRC.regDate);
  ", .con = con)
        
        result = DBI::dbExecute(con, merQuery)
        if (result > 0) {
          cat(sprintf("[SUCC] result : %s / uuidInfo : %s", result, uuidInfo), "\n")
        } else {
          cat(sprintf("[ERROR] result : %s / uuidInfo : %s", result, uuidInfo), "\n")
        }
        
      }
      , warning = function(warning) {cat(sprintf("[WARN] %s", warning), "\n")}
      , error = function(error) {cat(sprintf("[ERROR] %s", error), "\n")}
    )
  }
}
