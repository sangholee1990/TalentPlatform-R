...
for (pageInfo in pageList) {
  ...
  for (uuidInfo in uuidList) {
    ...
    # 1) Actor/Organisation name
    acrOrgName = tryCatch({
      sprintf("%s[%s]", manDtlInfo$name, manDtlInfo$names$texts$language$isoCode %>% toupper()) %>% errorHander()
    }, error = function(e) {NA})
    
    # 2) Actor ID/SRN
    actIdSrn = tryCatch({
      sprintf("%s", manDtlInfo$srn) %>% errorHander()
    }, error = function(e) {NA})
    
    # 3) Applicable legislation
    appLeg = tryCatch({
      sprintf("%s", resCodeA[resDtlDevData$legislation$code] %>% unlist()) %>% errorHander()
    }, error = function(e) {NA})
    
    # 4) UDI-DU/EUDAMED DI/ Issuing entity
    udiDu = tryCatch({
      issEnt = resCodeA[resDtlDevData$basicUdi$issuingAgency$code] %>% unlist()
      sprintf("%s / %s", resDtlDevData$basicUdi$code, issEnt) %>% errorHander()
    }, error = function(e) {NA})
    
    # 5) Risk class`refdata.risk
    riskCls = tryCatch({
      sprintf("%s", resCodeA[resDtlDevData$riskClass$code] %>% unlist()) %>% errorHander()
    }, error = function(e) {NA})
    
    # 6) Device name
    devName = tryCatch({
      sprintf("%s", resDtlDevData$deviceName) %>% errorHander()
    }, error = function(e) {NA})
    
    # 7) UDI-DI code/Issuing entity
    udiDi = tryCatch({
      sprintf("%s / %s", resDtlToolData$primaryDi$code, resCodeA[resDtlToolData$primaryDi$issuingAgency$code] %>% unlist()) %>% errorHander()
    }, error = function(e) {NA})
    
    # 8) Status,Nomenclature code(s)
    status = tryCatch({
      tmpCode = (resDtlToolData$cndNomenclatures$description$texts %>% unlist())["text"] %>% tolower()
      sprintf("%s: %s", resDtlToolData$cndNomenclatures$code, paste0(toupper(substring(tmpCode, 1, 1)), substring(tmpCode, 2))) %>% errorHander()
    }, error = function(e) {NA})
    
    # 9) Name/Trade name(s)
    tradeName = tryCatch({
      sprintf("%s [%s]", resDtlToolData$tradeName$texts$text, resDtlToolData$tradeName$texts$language$isoCode %>% toupper()) %>% errorHander()
    }, error = function(e) {NA})
    
    # 10) Member state of the placing on the EU market of the device
    memberState = tryCatch({
      sprintf("%s", resDtlToolData$placedOnTheMarket$name) %>% errorHander()
    }, error = function(e) {NA})
    
    # 11) Basic UDI-DI details 섹션 내 Version
    version = tryCatch({
      sprintf("Version %s", resDtlDevData$versionNumber) %>% errorHander()
    }, error = function(e) {NA})
    
    # 12) Basic UDI-DI details 섹션 내 Last update date
    lastUpdData = tryCatch({
      sprintf("%s", resDtlToolData$deviceStatus$statusDate) %>% errorHander()
    }, error = function(e) {NA})
    
    # 13) UDI-DI details 섹션 내 additional product description
    addProDesc = tryCatch({
      sprintf("%s [%s]", resDtlToolData$additionalDescription$text$text, resDtlToolData$additionalDescription$text$language$isoCode %>% toupper()) %>% errorHander()
    }, error = function(e) {NA})
