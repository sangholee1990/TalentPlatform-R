...
```{r}
data_r <- data_r %>%
  mutate(interference = rowMeans(select(., c(4,6)), na.rm = TRUE))%>%
  select(-c(4,6))
```
...
data_r %>%
  ggplot() +
  geom_boxplot(mapping = aes(x=interference, y=avgtime))+
  labs(x="interference", y ="average time", title="Boxplot social media usage time according to social media interference level")
```
...
data_r %>%
  ...
  labs(x="dependence", y ="average time", title="Violinplot of social media usage time according to social media interference level")
```
...
- 그 결과 comparison, interference, mental_health, dependence 순으로 높은 양의 관계 (0.20 ~ 0.44)를 띠는 반면 age에서는 음의 관계성 (-0.38)을 보임
...
```{r, warning=FALSE}
modelData = data_r %>% 
  dplyr::select(age, comparison, dependence, interference, mental_health, avgtime_numeric) %>% 
  as.tibble()
...
# 선형회귀모형에서 변수선택
- 훈련 데이터에서 독립변수 (age, comparison, dependence, interference, mental_health) 및 종속변수 (avgtime)를 선정하여 선형회귀모형을 수행함
