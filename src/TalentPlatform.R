...
# 요구사항
#================================================
# R을 이용한 오쿤의 법칙 적용 및 시각화

# 문제 1: 오쿤의 법칙
# 위 문제에서는 오쿤의 법칙을 다른 나이와 성별 집단으로 분류하여 분석하는 것을 필요로 한다. 
# 데이터 data_3_okun.csv는 1980년 2분기에서 2014년 4분기까지의 분기별 실업률과 실질 로그 GDP를 반영하고 있다. 
# 실업률의 경우 5개의 집단(15-24살, 25-34살,35-44살, 45-55살, 그리고 55살 이상)으로 두 성별 모두 분류되어 있다.
# 기본적으로 다음의 선형회귀모형을 시계열변수와 함께 고려하라.
# 
# ut-ut_1= B0+B1(yt-yt_1)+Et
# 
# ut와 yt는 각각 시간 t에 있어서의 실업률과 실제 생산량을 의미한다. 
# 가장 최선의 모형이 무엇이지 식별하고, 그 모형을 가장 적합하다고 생각하는 방법으로 측정해보라. 
# 고전적 가정이 맞는지 또한 생각해볼 필요가 있다는 것을 명심하라. 적어도 10개 이상의(적어도 5개 남자, 5개 여자로 구성하여) 실증적인 결과를 보고해야 한다.
# 이러한 분석에 기반하여, 짧은 보고서를 작성하라. 보고서의 끝 부분에는 한국 정부에게 실업률과 관련하여 시정할 정책에 대한 조언이 꼭 담겨야 한다. 
# 이에 대한 논증을 확실히 해서 보고하라. 

# 1. 실업률 
# -자료 출처 : 한국 통계청
# -data : 1980q1-2014q4(qurterly)
# 
# 2. gdp 
# -real, seasonally adjustedm national currency
# (계절조정, 실질, 분기)
# -단위 십억원
# -자료출처 : 한국은행
...
fileInfo = Sys.glob(paste(globalVar$inpPath, "LSH0182_data_3_Okun.csv", sep = "/"))
data = readr::read_csv(file = fileInfo)
...
#*****************************************************************************
dataL1 = data %>% 
  # dplyr::select(all, ln_gdp_100) %>%
  dplyr::mutate(
    ut = all - lag(all)
    , yt = 100*(ln_gdp_100 / lag(ln_gdp_100) - 1)
  ) %>% 
  dplyr::select(-ln_gdp_100)

tsData = ts(dataL1, start=c(1980,2), end=c(2014,4), frequency=4) %>% 
  na.omit()

#*******************************************************
# 생산량 및 실업률을 포함하여 회귀식 구성
#*******************************************************
lmFit = lm(yt ~ ut, data = tsData)
summary(lmFit)

# 실질 경제성장률의 계수 (beta의 계수)
# = exp(-0.03682)
# = 0.9638496

# 즉 실질 GDP 성장률이 1% 증가하면 실업률이 0.96% 감소
# 해당 계수의 P값 (0.008767)은 0.01 이하로서 신뢰구간 95% 가정하에서 통계적으로 유의함

# 이러한 결과를 토대로 대한민국도 오쿤의 법칙이 성립함
# 오쿤의 계수은 1% 상승 시 감소되는 경제성장률 정도로서 beta 계수의 역수

# 오쿤의 계수 
# = 1 / exp(-0.03682)
# = 1.037506
# 즉 실업률 1% 증가하면 실질 GDP 성장률 1.04% 감소

#*******************************************************
# 세부 설명 
#*******************************************************
# 대한민국 경기변동에 따른 노동시장의 반응을 살펴보기 위해 
# 1980년 2분기에서 2014년 4분기 동안의 자료를 이용하여 오쿤의 법칙을 수행함.
# 국내 기술조사에 따르면 약 30년간('71년부터 '98년) 성장률과 실업률의 추세는 경제가 1% 성장하면 
# 실업률은 약 0.28% 하락하는 관계를 보여 왔음. 
# 이를 두 기간으로 나누어 보면, '80년 이전은 성장 1%에 실업률 0.17% 하락,
# '81년 이후는 성장 1%에 실업률 0.32% 하락하는 관계를 보임. 
# 특히 근래에 올수록 경제성장에 대해 실업률 (오쿤의 계수 1.03)이 민감하게 반응하고 있음.

# 또한 오쿤의 법칙이 성립함에 따라 경제성장을 통해 실업을 해소할 수 있
# 는 가능성은 있으나  
# 경제구조의 변화에 따라 실업이 더 이상 소득의 변화에만 의존하지 않는다는 것을 인식해야 함

# 특히 대한민국의 경우 경영상의 이유로 인한 해고의 도입 등 노동시장 유연성 제고가 '97년 이후로 본격적으로 시행되었기 때문에 
# 근래에 올수록 경기에 대한 실업률의 민감한 변화가 외국의 사례 (80년 이후 경기변도에 대해 실업률이 민감하게 반응)와 같이 해석될 수는 없음.
# 이는 다른 부분(단시간 근로자비율의 증가, 성과금제 도입을 통한 임금유연성 증가 등)에서 노동시장의 유연성이 증가한 결과로 잠정적인 해석이 가능한 것으로 판단됨.

# 더 나아가 경제성장률과 실업률의 추이에 의하면 90년대의 외환위기 이전과 이후에 따라 그 추이가 상이한 형태를 나타남
# 즉 분석기간의 초반에는 높은 실업률과 낮은 경제 성장률의 추이이나 중반에는 두 변수가 큰 편차를 보이지 않음 
# 그리고 외환위기 이후에는 경제성장률과 실업률이 초반의 추이로 회귀되나 확대된 두 변수의 추이가 일정한 형태를 유지함
# 향후 오쿤의 법칙을 분석함에 있어서 이와 같은 구조적 변화를 구분하여 분석하는 것이 필요함

saveImg = sprintf("%s/%s_%s.png", globalVar$figPath, serviceName, "생산량 및 실업률을 이용하여 오쿤의 법칙 시각화")
...
ggscatter(
  ...
  labs(
    title = NULL
    , x = "∆ in Unemployment Rate (Percentage Points)"
    , y = "∆% in real GDP"
    , subtitle = "생산량 및 실업률을 이용하여 오쿤의 법칙 시각화"
  ) +
  theme(
    text = element_text(size = 18)
    ) +
...
#*******************************************************
# 5개의 나이, 2개의 성별, 생산량, 실업률을 포함하여 회귀식 구성
#*******************************************************
lmFit = lm(yt ~ ., data = tsData)
# AIC 기준으로 변수 선택
stepAic = MASS::stepAIC(lmFit, direction = "both")
...
# 최적의 회귀모형
# 종속변수: yt
# 독립변수: a15_24 + a25_34 + a55 + m15_24 + m25_34 + `m55-` + f15_24 + f25_34 + `f55-` + ut + m35_44

# 실질 경제성장률의 계수 (beta의 계수)
# = exp(-0.04883)
# = 0.952343

# 즉 실질 GDP 성장률이 1% 증가하면 실업률이 0.95% 감소
# 해당 계수의 P값 (1.203e-10)은 0.01 이하로서 신뢰구간 95% 가정하에서 통계적으로 유의함
