...
sheetName = dplyr::case_when(
  sheetInfo == 1 ~ "졸본-국내성15개"
  , sheetInfo == 2 ~ "장수평양이후7개"
  , sheetInfo == 3 ~ "환도성-동황성7개"
  , TRUE ~ NA_character_
)
...
# 시트 4 : 졸본-국내성15개_환도성-동황성2개
sheetInfo = 4
sheetList = c(4)

# 시트 5 : 장수평양이후7개_환도성-동황성5개
sheetInfo = 5
sheetList = c(5)
...
sheetName = dplyr::case_when(
  ...
  , sheetInfo == 2 ~ "장수평양이후7개"
  , sheetInfo == 3 ~ "환도성-동황성7개"
  , sheetInfo == 4 ~ "졸본-국내성15개_환도성-동황성2개"
  , sheetInfo == 5 ~ "장수평양이후7개_환도성-동황성5개"
  , TRUE ~ NA_character_
)
...
# setBreakCont = c(seq(0.48, 0, -0.01))
# setBreakText = c(seq(0.48, 0.10, -0.01))

# setBreakCont = c(seq(0.51, 0, -0.04))
# setBreakText = c(seq(0.51, 0.10, -0.04))
# 
# setBreakCont = c(seq(0.48, 0, -0.04))
# setBreakText = c(seq(0.48, 0.10, -0.04))
# 
# setBreakCont = c(seq(0.65, 0, -0.04))
# setBreakText = c(seq(0.65, 0.10, -0.04))

# setBreakCont = c(seq(0.53, 0, -0.04))
# setBreakText = c(seq(0.53, 0.10, -0.04))
...
# 평균식분도에 대한 최대값입니다.
# 환도성-동황성7개 : 0.65598583

# 평균식분도에 대한 최대값입니다.
# 졸본-국내성15개_환도성-동황성2개 : 0.5378074

# 평균식분도에 대한 최대값입니다.
# 장수평양이후7개_환도성-동황성5개 : 0.5170380
