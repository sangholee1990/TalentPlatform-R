...
reqDataKey = "bf9fH0KLgr65zXKT5D/dcgUBIj1znJKnUPrzDVZEe6g4gquylOjmt65R5cjivLPfOKXWcRcAWU0SN7KKXBGDKA=="

# 요청 URL
reqUrl = "http://openapi.molit.go.kr/OpenAPI_ToolInstallPackage/service/rest/RTMSOBJSvc/getRTMSDataSvcAptTradeDev"
...
codeInfo = Sys.glob(file.path(globalVar$inpPath, serviceName, "법정동코드_전체자료.txt"))
...
fileInfo = Sys.glob(paste(globalVar$inpPath, serviceName, "가구_특성정보_(+소득정보)_201211.csv", sep = "/"))
...
#     if (resItems == "") { next }
#
#     cat(sprintf("dtDate : %10s | code : %5s", sDate, codeList[j, 'emdCd']), "\n")
#
#     resItem = resItems$item %>%
#       as.data.frame() %>%
#       dplyr::mutate(
#         지번 = paste0(resItems$item$`지번`)
#         , 법정동본번코드 = paste0(resItems$item$`법정동본번코드`)
#       )
#
#     dataL1 = dplyr::bind_rows(
#       dataL1
#       , data.frame('dtYm' = sDate, 'emdCd' = codeDistList[j, 'emdCd'], resItem)
#     )
#
#   }
# }
...
# 자료 저장
#***********************************************
# saveFile = sprintf("%s/%s/seoul_apartment_transaction-%s-%s.csv", globalVar$inpPath, serviceName, min(dataL1$dtYm, na.rm = TRUE), max(dataL1$dtYm, na.rm = TRUE))
# readr::write_csv(x = dataL1, file = saveFile)

#***********************************************
# 데이터 전처리
#***********************************************
# fileInfo = Sys.glob(file.path(globalVar$inpPath, serviceName, "seoul apartment transaction.csv"))
fileInfo = Sys.glob(file.path(globalVar$inpPath, serviceName, "seoul_apartment_transaction-201701-202211.csv"))
...
ggplot(dataL2, aes(x = d2, y = val, color = d2)) +
  geom_boxplot() +
  labs(x = "법정동", y = "연소득당 거래금액", color = NULL, fill = NULL, subtitle = "법정동에 따른 연소득당 거래금액 상자 그림") +
...
saveFile = sprintf("%s/%s/%s.csv", globalVar$inpPath, serviceName, "seoul_apartment_transaction-addrData")
...
validData = data.frame(
  xAxis = predict(lmFitStep, newdata = dataL5)
  , yAxis = dataL5$val
  , type = "전체 아파트"
  # , type = "중형 아파트"
  # , type = "소형 아파트"
...
biasVal = Metrics::bias(validDataL1$xAxis, validDataL1$yAxis)
rmseVal = Metrics::rmse(validDataL1$xAxis, validDataL1$yAxis)

# 전체 아파트에 대한 주택가격 결정요인 (연소득당 거래금액) 예측 산점도
mainTitle = sprintf("%s", "전체 아파트에 대한 주택가격 결정요인 예측 산점도")
# mainTitle = sprintf("%s", "중형 아파트에 대한 주택가격 결정요인 예측 산점도")
# mainTitle = sprintf("%s", "소형 아파트에 대한 주택가격 결정요인 예측 산점도")

saveImg = sprintf("%s/%s/%s.png", globalVar$figPath, serviceName, mainTitle)

ggscatter(
  validDataL1, x = "xAxis", y = "yAxis", color = "black"
