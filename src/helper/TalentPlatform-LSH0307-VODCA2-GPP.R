...
fileInfo = Sys.glob(file.path(globalVar$inpPath, serviceName, "VODCA2GPP_v1.nc"))
...
posStatDataL1 = posStatData %>% 
  ...
  dplyr::mutate(
    dtDate = lubridate::make_date(year, month)
    , sYm = format(dtDate, "%Y-%m")
  )
...
timeList = timeInfo$sYm %>% unique() %>% sort()
# timeEle = timeList[1]
for (timeEle in timeList) {
  
  cat(sprintf("timeList : %s", timeEle), "\n")
  
  selTimeInfo = timeInfo %>% 
    dplyr::filter(sYm == timeEle)
  
  posStatDataL2 = posStatDataL1 %>%
    dplyr::filter(sYm == timeEle)
    
  
  extDataL1 = extData %>% 
    dplyr::select(c(selTimeInfo$rowid)) %>%
    rowSums(na.rm = TRUE) %>%
    as.tibble() %>% 
    magrittr::set_colnames("gpp") %>% 
  ...
  plotSubTitle = sprintf("[%s] %s (%s-%s)", timeEle, "전구 전체 월간 총합", round(min(extDataL1$gpp), 0), round(max(extDataL1$gpp), 0))
  saveImg = sprintf("%s/%s_%s_%s.png", globalVar$figPath, serviceName, keyInfo, plotSubTitle)
  
  makemMecarPlot = ggplot() +
    coord_fixed(ratio = 1) +
    geom_tile(data = extDataL2, aes(x = lon, y = lat, fill = gpp)) +
  ...
  plotSubTitle = sprintf("[%s] %s (%s-%s)", timeEle, "북반구 전체 월간 총합", round(min(extDataL1$gpp), 0), round(max(extDataL1$gpp), 0))
  saveImg = sprintf("%s/%s_%s_%s.png", globalVar$figPath, serviceName, keyInfo, plotSubTitle)
  
  makePlot = PlotSvalbard::basemap(limits = 30, grid.size = 0.2, label.font = 16) +
    geom_point(
      data = ggOceanMaps::transform_coord(extDataL2, lon = "lon", lat = "lat", bind = TRUE)
      , aes(x = lon.proj, y = lat.proj, colour = gpp, fill = gpp)
      , shape = 15
    ) +
    geom_point(
      data = ggOceanMaps::transform_coord(posStatDataL2, lon = "lon", lat = "lat", bind = TRUE)
...
timeList = timeInfo$year %>% unique() %>% sort()
# timeEle = timeList[1]
...
for (timeEle in timeList) {
  ...
  if (nrow(meanData) == 0) {
    ...
    meanData = dplyr::bind_cols(meanData, extDataL1)
  }
}
...
for (timeEle in timeList) {
  ...
  if (nrow(clmData) == 0) {
    ...
    clmData = dplyr::bind_cols(clmData, extDataL1)
  }
}
...
for (timeEle in timeList) {
  ...
  plotSubTitle = sprintf("[%s] %s (%s-%s)", timeEle, "북반구 전체 월간 총합", round(min(extDataL2$gpp), 0), round(max(extDataL2$gpp), 0))
  saveImg = sprintf("%s/%s_%s_%s.png", globalVar$figPath, serviceName, keyInfo, plotSubTitle)
...
for (timeEle in timeList) {
  ...
  clmDataL2 = dplyr::bind_cols(clmDataL1, extDataL1) %>% 
  ...
  plotSubTitle = sprintf("[%s] %s (%s-%s)", timeEle, "전구 전체 월간 아노말리", round(min(clmDataL2$gpp, na.rm = TRUE), 0), round(max(clmDataL2$gpp, na.rm = TRUE), 0))
  saveImg = sprintf("%s/%s_%s_%s.png", globalVar$figPath, serviceName, keyInfo, plotSubTitle)
  ...
  plotSubTitle = sprintf("[%s] %s (%s-%s)", timeEle, "북반구 전체 월간 아노말리", round(min(clmDataL3$gpp, na.rm = TRUE), 0), round(max(clmDataL3$gpp, na.rm = TRUE), 0))
  saveImg = sprintf("%s/%s_%s_%s.png", globalVar$figPath, serviceName, keyInfo, plotSubTitle)
...
plotSubTitle = sprintf("[%s] %s (%s-%s)", "1998-2020", "전구 전체 Theil-Sen slope", round(min(clmDataL3$slope, na.rm = TRUE), 0), round(max(clmDataL3$slope, na.rm = TRUE), 0))
saveImg = sprintf("%s/%s_%s_%s.png", globalVar$figPath, serviceName, keyInfo, plotSubTitle)
...
plotSubTitle = sprintf("[%s] %s (%s-%s)", "1998-2020", "전구 전체 Theil-Sen slope 90", round(min(clmDataL4$slope, na.rm = TRUE), 0), round(max(clmDataL4$slope, na.rm = TRUE), 0))
saveImg = sprintf("%s/%s_%s_%s.png", globalVar$figPath, serviceName, keyInfo, plotSubTitle)
